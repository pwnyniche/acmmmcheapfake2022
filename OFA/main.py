...
def calculate_entail_score(row):
    with torch.no_grad():
...
def calculate_entail_score_task_2(row):
    if not os.path.exists(os.path.join(IMAGE_DATA_DIR,row["img_local_path"])):
        row["img_local_path"] = row["img_local_path"].replace('.jpg','.png')
    with torch.no_grad():
        d = construct_sample(row["img_local_path"],
            Image.open(os.path.join(IMAGE_DATA_DIR,row["img_local_path"])),
            row['caption'], '','entailment')
        sample = collate([d], pad_idx, eos_idx)
        result = eval_snli_ve(task,None,models,sample)
    return result[0][0]['e_score'].numpy().tolist()

def run(df):
    tqdm.pandas()
    z=df.progress_apply(lambda x: calculate_entail_score(x), axis=1)
    z.to_csv('pred_ofa.csv', index=False)

def run_task_2(df):
    torch.manual_seed(0)
    tqdm.pandas()
    df['ofa_score'] = df.progress_apply(lambda x: calculate_entail_score_task_2(x), axis=1)
    return df